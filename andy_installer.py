...
class Andy:
    ...
    def run_installation(self):
        ...
        self.execute_command("sudo rm -rf /opt/llmui-core", "Nettoyage ancien install", 5)
        success, _ = self.execute_command(
            f"sudo git clone {self.github_repo} /opt/llmui-core",
            "Clone dépôt",
            5,
            critical=True
        )
        ...
        self.execute_command(
            "cd /opt/llmui-core && sudo python3 -m venv venv",
            "Création venv",
            5,
            critical=True
        )
        ...
        self.execute_command(
            "cd /opt/llmui-core && sudo venv/bin/pip install --upgrade pip",
            "Upgrade pip",
            5
        )
        
        self.execute_command(
            "cd /opt/llmui-core && sudo venv/bin/pip install -r requirements.txt",
