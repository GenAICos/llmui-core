...
class Andy:
    ...
    def call_ollama(self, prompt, max_tokens=500):
        ...
        try:
            ...
            self.log(f"🤖 Andy réfléchit avec {self.llm_model}...", "INFO")
    ...
    def fix_requirements_txt(self, error_message, requirements_path="/opt/llmui-core/requirements.txt"):
        ...
        prompt = f"""You are Andy, a DevOps AI assistant. A Python package installation failed with this error:
...
TASK: Fix the requirements.txt file. Output ONLY the corrected lines that need to be changed, in this exact format:

FIXES:
old_package_line -> new_package_line

Rules:
1. Update version constraints to match available versions
2. Remove version constraints that are too restrictive
3. For torch/torchvision, use latest stable versions (>=2.5.0)
4. Keep other dependencies unchanged unless they conflict
5. Output ONLY the lines that need changes
...
Now provide the fixes:"""
        ...
        if not response:
            self.log("Andy n'a pas pu analyser l'erreur", "ERROR")
            return False
        ...
        if not fixes:
            self.log("Andy n'a pas trouvé de corrections à appliquer", "WARNING")
            return False
        ...
        for old_line, new_line in fixes:
            if old_line in updated_requirements:
                ...
                self.log(f"  ⚠ Ligne non trouvée: {old_line}", "WARNING")
        ...
        while not pip_success and retry_count < self.max_retries:
            ...
            if success:
                ...
                if retry_count < self.max_retries:
                    ...
                    self.log("❌ Échec après 3 tentatives", "ERROR")
