...
def main():
    ...
    # Copie des fichiers (inchangé)
    log("Copie des fichiers source vers /opt/llmui-core/...", "INFO")
    
    if os.path.exists(f"{temp_dir}/src"):
        execute_command(f"sudo cp -r {temp_dir}/src /opt/llmui-core/", "Copie répertoire src")
    if os.path.exists(f"{temp_dir}/web"):
        execute_command(f"sudo cp -r {temp_dir}/web /opt/llmui-core/", "Copie répertoire web")
    if os.path.exists(f"{temp_dir}/scripts"):
        execute_command(f"sudo cp -r {temp_dir}/scripts /opt/llmui-core/", "Copie répertoire scripts")

    if os.path.exists(f"{temp_dir}/config.yaml.example"):
        if os.path.exists("/opt/llmui-core/config.yaml"):
            execute_command(
                f"sudo cp {temp_dir}/config.yaml.example /opt/llmui-core/config.yaml.example",
                "Copie config.yaml.example (config existe déjà)"
            )
        else:
            execute_command(
                f"sudo cp {temp_dir}/config.yaml.example /opt/llmui-core/config.yaml",
    ...
    # ====================== AJOUTS DEMANDÉS ======================
    log("Application des corrections finales demandées...", "INFO")
    ...
    execute_command(
        ...
        "Copie config_yaml.example → config.yaml"
    )
